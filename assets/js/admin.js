...
function attachAdminData() {
  const affUnsub = affDb.collection('affiliates').onSnapshot((snap) => {
    ...
    renderReferrals(); // nama afiliator mungkin berubah, render ulang
  });
...
function renderReferrals() {
  ...
  admLatestReferrals.forEach((d) => {
    ...
    tr.innerHTML = `
      ...
      <td><input type="number" data-field="commission" value="${d.commissionAmount || 0}"></td>
  ...
  tbody.querySelectorAll('button[data-ref-id]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      const commission = parseFloat(row.querySelector('input[data-field="commission"]').value) || 0;
      btn.textContent = 'Menyimpan...';
      try {
