...
function setAffMode(mode) {
  ...
  if (mode === 'login') {
    ...
    affAuthSub.textContent = 'Dapatkan kode referral pribadimu.';
  ...
  const bizUnsub = affDb.collection('affiliates').doc(uid).onSnapshot((doc) => {
    ...
    document.getElementById('aff-wa-share').href =
      'https://wa.me/?text=' + encodeURIComponent(
        `Halo! Kalau usahamu butuh aplikasi kasir, stok, atau sistem sendiri, coba cek Pardi ya: ${link}`
      );

    // Listener referral setelah kita tahu code-nya
